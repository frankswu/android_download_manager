...
import example.filedownload.pub.DownloadManager;
...
public class FileDownloadActivity extends ListActivity {
	...
	private DownloadTaskListener downloadTaskListener = new DownloadTaskListener() {
		...
		public void updateProcess(DownloadTask mgr) {
			Log.i("updateProcess:",	mgr.toString());
		...
		public void finishDownload(DownloadTask mgr) {
			Log.i("finishDownload:",	mgr.toString());
			for (int i = 0; i < Utils.url.length; i++) {
				if (Utils.url[i].equalsIgnoreCase(mgr.getUrl())) {
	...
	private Runnable runnable = new Runnable() {
		...
		public void run() {
			while (!Utils.isNetworkAvailabel(FileDownloadActivity.this)) {
				Toast.makeText(FileDownloadActivity.this, "网络已断开",
						Toast.LENGTH_LONG);
...
//				startDownload(msg.arg1);
				DownloadManager.getInstance(FileDownloadActivity.this).startDownload(Utils.url[msg.arg1], Utils.APK_ROOT, Utils.getFileNameFromUrl(Utils.url[msg.arg1]), downloadTaskListener);
				break;
			case MSG_STOP_DOWNLOAD:
//				stopDownload(msg.arg1);
//				DownloadManager.getInstance(FileDownloadActivity.this).s
				break;
			case MSG_PAUSE_DOWNLOAD:
//				pauseDownload(msg.arg1);
				DownloadManager.getInstance(FileDownloadActivity.this).pauseDownload();
				break;
			case MSG_CONTINUE_DOWNLOAD:
//				continueDownload(msg.arg1);
				DownloadManager.getInstance(FileDownloadActivity.this).continueDownload(Utils.url[0], Utils.APK_ROOT, Utils.getFileNameFromUrl(Utils.url[0]), downloadTaskListener);
				break;
	...
	public void updateDownload(int viewPos, DownloadTask mgr) {
		...
		view.setText("" + (int) mgr.getDownloadPercent() + "%" + " "
				...
				+ Utils.size(mgr.getDownloadSize()) + "/"
...
//			file.delete();
		try {
			tasks[viewPos] = new DownloadTask(this, Utils.url[viewPos],	Utils.APK_ROOT, downloadTaskListener);
	...
	private class ListAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				convertView = View.inflate(this.context, R.layout.list_item,
						null);
				viewList.add(convertView);

				Button btnStart = (Button) convertView
						.findViewById(R.id.btn_start);
				Button btnPause = (Button) convertView
						.findViewById(R.id.btn_pause);
				Button btnStop = (Button) convertView
						.findViewById(R.id.btn_stop);
